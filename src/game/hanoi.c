...
void TowerOfHanoi(Map* MapTowerOfHanoi){
    ...
    do{
        ...
        while (checkCondition()){
            ...
            while (isEmptyHanoi(asal = fromAsal())){
                draw();
...
void menu(){ //udah bener
	...
	printf("WARNING!!!\n");
	printf("Sesuaikan ukuran terminal anda dengan ukuran tower yang anda pilih\n");
	//scanf("%d", &tinggi);
...
void moved (int asal, int tujuan){
    ...
    int nilai = PopHanoi(arrAsal);
    PushHanoi(arrTujuan, nilai);
    if (asal != tujuan){
        moves++;
    }
}
